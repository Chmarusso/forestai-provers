...
contract ForestSlasher is Ownable, Pausable {
    ...
    function commit(bytes32 _commitHash, address _valAddr, address _ptAddr) public whenNotPaused {
    ...
    function reveal(bytes32 _commitHash, address _valAddr, address _ptAddr, ProviderScore[] memory _provScores) public whenNotPaused {
        ...
        delete granularScore.provScores;
        // Copy each element individually
        for (uint i = 0; i < _provScores.length; i++) {
            granularScore.provScores.push(_provScores[i]);
        }
        granularScore.status = CommitStatus.REVEALED;

        emit CommitRevealed(_valAddr, _ptAddr, _commitHash, currentEpochEndBlockNum);
    }
    ...
    /// @notice Gets the number of PTs that have submitted commits for the currently processed epoch
    /// @return ptNum Number of PTs that have submitted commits for the currently processed epoch
    function getPtNumThisEpoch() external view returns (uint256) {
        return currentPtAddresses.length;
    }
